...
..  glossary::
    ...
    Markup Language
    Markup Languages
        Simple languages that let you write text documents with minimal rules,
        then later process those documents to build nice looking web pages, or
        other output products. Popular markup languages include
        reStructuredText_, and MarkDown_.

    Virtual Environment
        A working environment, isolated fro the rest of your development
        system, wher you can work on a project. Typically, we install tools
        needed for this project inside this environment and do not impact other
        tools installed on the system. One way to set up such an environment is
        to use a :term:`virtual machine` for a project.
    ...
    Markdown
        Another popular text based markup language. Similar to
        reStructuredText_ but i think it is not as nice looking in raw form.
    ...
    Branch
    Branching
        Start a new development track, separate from the parent development track.

    Merge
    Merging
        Combine two (or more) development tracks into one. This can lead to :term:`conflicts` if you are not careful!

    Conflict
    Conflicts
        When changes to one section of code made by two (or more) developers)
        conflict with each other. Someone has to decide which of the available
        changes to apply. This is usually a human!  ..  vim: set filetype=rst
        spell

    Combinational
        A circuit where the output is a pure function of the input.

    Propogation Delay
        The time it takes for a :term:`combinational` digital component
        to convert all inputs to the final output.

    Sequential
        A digital circuit where the output depends on the inputs and a
        value maintained in an internal state variable.

    Memory
        A collection of digital devices that can hold the data we process in
        our programs. Memory is usually (but not always) organized as a linear array of
        :term:`bytes`, each of which can be accessed by referring to them using an
        :term:`address`. (:cite:`DictRef3`)

    Byte
    Bytes
        An eight-bit sequence of binary digits. (:cite:`DictRef4`)

    Address
        An unsigned integer number specifying a location in a computer system's memory (:cite:`DictRef5`)
